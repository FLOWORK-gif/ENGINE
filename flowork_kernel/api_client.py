...
class ApiClient:
    """
    A client to interact with local and remote APIs.
    [MODIFIED V5 - FINAL] All authentication logic now correctly and solely points to Supabase.
    """
    ...
    def login_user(self, email, password):
        ...
        try:
            ...
            final_response = {
                ...
                "tier": user_profile.get('tier', 'free') # This will now be the correct tier!
            }
    ...
    def get_user_profile_by_token(self, session_token: str):
        """
        Validates a session token and fetches the full user profile, including tier.
        """
        try:
            ...
            if not user_success:
                return False, user_data # This means token is invalid or expired
    ...
    def activate_license(self, license_content: dict):
        """
        Sends a license activation request to the local API server.
        """
        try:
    ...
    def deactivate_license(self):
        """
        Sends a license deactivation request to the local API server.
        """
        try:
    ...
    def restart_application(self):
        """
        Sends a request to the server to trigger an application restart event.
        """
        try:
